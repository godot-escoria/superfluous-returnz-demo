...
var state_machine = preload("res://addons/sr-dialog-simple/esc_dialog_simple_state_machine.gd").new()
...
func _init_type_player(type: String) -> void:
	if type == "floating":
		_type_player = preload(\
			"res://addons/sr-dialog-simple/types/floating.tscn"\
		).instantiate()
	else:
		_type_player = preload(\
			"res://addons/sr-dialog-simple/types/avatar.tscn"\
		).instantiate()
...
func do_choose(dialog_player: Node, dialog: ESCDialog, type: String = "simple"):
	...
	if type == "simple" or type == "":
		chooser = preload(\
			"res://addons/sr-dialog-simple/chooser/simple.tscn"\
		).instantiate()
